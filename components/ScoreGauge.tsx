...
export default function ScoreGauge({ total, className }: { total: number; className?: string }) {
  ...
  return (
    <div className={`flex h-full flex-col items-center gap-2 py-3 ${className ?? ""}`}>
      <style>{`
      ...
      <div className="relative w-full flex-1">
        <div
          ...
          aria-valuenow={total}
          aria-label="밀알 누적 점수"
          className="absolute inset-y-0 left-1/2 w-2.5 -translate-x-1/2 overflow-hidden rounded-full bg-[#efe3cf]"
        >
          <div
